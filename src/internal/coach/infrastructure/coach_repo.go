...
func (pcr *PostgresCoachRepo) GetChatHistory(ctx context.Context, userID string) ([]domain.ChatMessage, error) {
	...
	domChatMsgs := make([]domain.ChatMessage, len(chat_messages))
	for i, chatMsg := range chat_messages {
		domChatMsgs[i] = FromChatInfraToDomain(chatMsg)
	}

	return domChatMsgs, nil
}
